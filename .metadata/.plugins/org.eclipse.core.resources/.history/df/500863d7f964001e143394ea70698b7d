package com.OrangeHRMWebAppTestCases;
...
public class Ohrm_MutlipleUserCredentialsLogin_TestNg_PropFile extends BaseTest{
	...
	public void orangeHrmWebApplicationLoginPage() throws IOException {
		...
		ohrmMultiUserCredentialsExcel= new FileInputStream("./src/main/java/com/OrangeHRMTestDataFiles/1LogInTest.xlsx");
...
@Test(priority =2, description="Validation of Multiple User Credentials")
		...
		public void multiUserCredentialsLogin() throws IOException, InterruptedException {
			...
			{		
				...
				WebElement userNameElement=driver.findElement(userNameProperty);userNameElement.sendKeys(enteredUserName);
				By passwordProperty=By.id(prop.getProperty("orangeHRMApplicationLogInPage_Password_idProperty"));
				passwordElement =driver.findElement(passwordProperty);passwordElement.sendKeys(enteredPassword);
...
//LogUtility.info("The invalid User Credentials Combination is:- "+enteredUserName+" |"+enteredPassword);	
				}
...
//LogUtility.info("The Valid User Crendentials Combination is :- "+enteredUserName+" |"+enteredPassword);
...
//LogUtility.info("		Successfully Navigated to the OrangeHrm HomePage:- PASS");						
						}
						else {
...
//LogUtility.info("Validation of Multiple UserCredentials is Completed with Validation after Logout");
			...
			FileOutputStream mutlipleUsersTest = new FileOutputStream("./src/main/java/com/OrangeHRMTestResultFiles/OHRM_MultipleUsers_Validation.xlsx");
